class Randots {
  ...
  paint(ctx, geom, properties) {
    ...
    const alphaIncrement = parseFloat(
      properties.get("--randots-alpha-increment")
    );
    const probabilityIncrement = parseFloat(
      properties.get("--randots-probability-increment")
    );
    const hexColor = properties
      .get("--randots-fill-color")
      .toString()
      .split("#")
      .pop();
    let probability = parseFloat(
      properties.get("--randots-starting-probability")
    );
    let colorParts = {
    ...
    for (let line = 0; line < lines; line += tileSize) {
      ctx.fillStyle = `rgb(${colorParts.r}, ${colorParts.g}, ${colorParts.b}, ${
        colorParts.a
      })`;
